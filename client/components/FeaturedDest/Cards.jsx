...
const Card = ({outerWidth, outerHeight, innerLeft, innerTop, textSize, smallTextSize, numberActiv, topText, rating, img}) => {
    return (
        <div class="flex relative w-full h-full rounded-[15px] hover:scale-105 transition-all duration-200 ease-in-out hover:shadow-2xl hover:shadow-c3 hover:shadow-opacity-50 hover:shadow-offset-2 hover:shadow-offset-c00 cursor-pointer over:opacity-90" style={{width: `${outerWidth}`, height: `${outerHeight}`}}>
            <Image class="w-full h-full object-cover rounded-[15px] shadow-xl" src={img} alt='Picture' />
            <div class="absolute flex" style={{left: `${innerLeft}`, top: `${innerTop}`, right: `${innerLeft}`, bottom: `${innerTop}`}}>
                ...
                <div class="absolute left-0 w-full bottom-0">
                    <span class={`text-white font-bold font-dm-sans`} style={{fontSize: `${smallTextSize}`}}>{`${numberActiv} Activities`}</span>
                </div>
